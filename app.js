 ...
 *   · 工具：选择 / 直线 / 圆 / 文字 / 距离 / 角度 / 周长面积 / 框选文字（含周边）
 *   · 对象捕捉：端点 / 中点 / 圆心 / 交点（轴线×图形交叉部位）
 * ============================================================= */
(function () {
  'use strict';
  ...
  function computeSnap(sx, sy) {
    ...
    // 交点捕捉：在靠近光标的实体之间求几何交点（轴线与图形交叉部位）
  ...
  function handlePick(sx, sy, wp) {
    switch (S.mode) {
      ...
      case 'm-angle':
        S.pts.push(wp);
        if (S.pts.length === 3) {
          var v1 = { x: S.pts[1].x - S.pts[0].x, y: S.pts[1].y - S.pts[0].y };
          var v2 = { x: S.pts[2].x - S.pts[0].x, y: S.pts[2].y - S.pts[0].y };
          var a1 = Math.atan2(v1.y, v1.x), a2 = Math.atan2(v2.y, v2.x);
          var da = (a2 - a1) * 180 / Math.PI; while (da < 0) da += 360; if (da > 180) da = 360 - da;
          var rr = Math.min(Math.hypot(v1.x, v1.y), Math.hypot(v2.x, v2.y)) * 0.4;
          R.overlays.push({ kind: 'poly', pts: [S.pts[1], S.pts[0], S.pts[2]], color: '#ffd24a', label: '角度 ' + da.toFixed(3) + '°' });
          R.overlays.push({ kind: 'arc', c: S.pts[0], r: rr, a0: Math.min(a1, a2), a1: Math.max(a1, a2), color: '#ffd24a' });
          say('角度 = ' + da.toFixed(4) + '°'); S.pts = []; R.render();
        } else hint('角度：顶点→第一点→第二点（已拾 ' + S.pts.length + '）');
        break;
      case 'm-area':
        S.pts.push(wp);
  ...
  function setMode(m) {
    ...
    var tips = {
      ...
      'draw-text': '文字：指定插入点', 'm-dist': '距离：指定第一点', 'm-angle': '角度：指定顶点',
  ...
  // ---------------------------------------------------------------- 字体预加载（SHX 矢量字形）
  // 浏览器下字体是异步 fetch 的：首屏先用系统字体近似，加载完成后重渲染换成真 SHX。
  ...
  function loadBuffer(buf, name) {
    ...
    S.doc = doc;
    // 异步预加载 SHX 字体：加载完成后自动重渲染换成真实矢量字形
    if (typeof ShxText !== 'undefined') {
      preloadFonts(doc)
