...
### 10. 线条随 zoom 缩放显隐（最终方案：2x 超采样）— 2026-08-20

**问题**：第 9 条按 dpr 吸附设备像素的方案在 125%/150% 缩放下仍有改善，但用户实测反馈**斜向细线（尺寸线、引线等）随 zoom 仍会出现灰↔白闪烁**，截图中箭头所指白线在某些缩放级别几乎消失。

**根因**：发丝线吸附只能让**水平/垂直**线稳定落在设备像素中心；对于斜线，无论如何调整顶点位置，线段必然跨越多行/多列设备像素，子像素覆盖率仍会在 zoom/pan 时跳变。此外，单像素线宽刚好处于 Canvas2D 抗锯齿的敏感区间，dpr 吸附无法消除斜线的闪烁。

**修复**：把整帧绘制改为 **2x 超采样渲染**：
- `render()` 创建/复用一块 2 倍设备分辨率的离屏 canvas（`W×H×dpr×2`）。
- 所有几何、填充、文字、高亮、overlays 先绘制到离屏 canvas（变换矩阵也乘以 2）。
- 最后用 `drawImage()` 线性缩放回显示 canvas。
- 这样每个显示像素由 4 个子像素平均，子像素覆盖率变化连续化，斜线/细线不再在「灰↔白」间跳变；同时文字和线宽视觉大小保持不变（因为同时放大了 2 倍再缩小）。
