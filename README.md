...
3. **「细线随 zoom 显隐」部分仍存在（已知显示问题）**：已通过 2x 超采样渲染大幅改善（每个显示像素由 4 个子像素平均，覆盖率连续化），水平/垂直发丝线基本稳定。但**部分复杂图纸在极端缩放下，斜向细线（尺寸线/引线等）仍可能出现轻微可见度变化（灰↔白）**。如需进一步消除，可把 `render()` 内的超采样倍数 `ss` 从 `2` 调到 `3`（性能换稳定）。
